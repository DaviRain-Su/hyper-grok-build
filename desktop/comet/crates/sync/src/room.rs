...
/// Establish `WebSocketStream<MaybeTlsStream<TcpStream>>` for `url`, doing the
/// TCP connect with Happy Eyeballs (race every resolved address, first success
/// wins) instead of tokio-tungstenite `connect_async`'s sequential connect.
/// A blackholed address family — classically a dead IPv6 route on a network
/// whose VPN only carries IPv4 (confirmed: `curl -6` to the edge times out
/// while `curl -4` works) — never completes and is cancelled when a winner is
/// returned, so the dial is not held hostage to the dead family for the whole
/// CONNECT_TIMEOUT on every attempt.
pub async fn dial_ws(url: &str) -> Result<WebSocketStream<MaybeTlsStream<TcpStream>>, SyncError> {
    let (tls, host, port) = split_ws_url(url)?;
    let tcp = dial_tcp_happy_eyeballs(&host, port).await?;
    let stream = if tls {
        let connector = native_tls::TlsConnector::new()
            .map_err(|e| SyncError::WebSocket(format!("tls connector: {e}")))?;
        let connector = tokio_native_tls::TlsConnector::from(connector);
        let tls_stream = connector
            .connect(&host, tcp)
            .await
            .map_err(|e| SyncError::WebSocket(format!("tls handshake to {host}: {e}")))?;
...
/// Parse `ws(s)://host[:port]/…` into (is_tls, host, port). Host may be a
/// bracketed IPv6 literal. Default port 443 (wss) / 80 (ws).
fn split_ws_url(url: &str) -> Result<(bool, String, u16), SyncError> {
    let (tls, rest) = if let Some(r) = url.strip_prefix("wss://") {
        (true, r)
    } else if let Some(r) = url.strip_prefix("ws://") {
        (false, r)
    } else {
        return Err(SyncError::WebSocket(format!("not a ws(s) url: {url}")));
    };
    ...
    let (host, port) = if let Some(after_open) = authority.strip_prefix('[') {
        match after_open.find(']') {
            Some(end) => {
                let h = &after_open[..end];
                let p = after_open[end + 1..]
                    .strip_prefix(':')
    ...
    if host.is_empty() {
        return Err(SyncError::WebSocket(format!("no host in ws url: {url}")));
    }
    Ok((tls, host, port))
}

/// Race `TcpStream::connect` over every resolved address and return the first
/// that succeeds. A blackholed address (packets dropped, no RST — the dead-IPv6
/// case) never completes and is dropped when a winner returns, so the dial is
/// not stuck waiting on the OS SYN retransmission timeout of a dead family.
async fn dial_tcp_happy_eyeballs(host: &str, port: u16) -> Result<TcpStream, SyncError> {
    let addrs: Vec<std::net::SocketAddr> = tokio::net::lookup_host((host, port))
        .await
        .map_err(|e| SyncError::WebSocket(format!("resolve {host}:{port}: {e}")))?
        .collect();
    if addrs.is_empty() {
        return Err(SyncError::WebSocket(format!("no addresses for {host}:{port}")));
    }
    let mut set = futures::stream::FuturesUnordered::new();
    for addr in addrs {
        set.push(TcpStream::connect(addr));
    }
    ...
    Err(SyncError::WebSocket(format!(
        "connect {host}:{port}: {}",
        last_err.map(|e| e.to_string()).unwrap_or_else(|| "no route".into())
    )))
}
