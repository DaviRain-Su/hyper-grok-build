...
async fn host_session(
    ...
    // Happy-Eyeballs dial (comet_sync::dial_ws): races resolved addresses so a
    // dead IPv6 route doesn't hang the device-room dial like connect_async did.
    let ws = comet_sync::dial_ws(url)
        .await
