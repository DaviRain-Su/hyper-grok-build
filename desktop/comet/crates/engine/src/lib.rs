...
pub use comet_harness::agent_login;
...
impl Engine {
    ...
    pub async fn assemble_runtime(
        ...
        // Release checker only when online. Local-link (offline) builds never
        // poll a cloud edge for updates — use the monorepo `hyper update` path.
        if online {
        ...
        tracing::info!(device_id = %core.device_id, online, "engine core assembled");
