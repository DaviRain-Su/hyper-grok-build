...
pub fn default_registry() -> HarnessRegistry {
    ...
    registry.register_lazy(
        HarnessDescriptor {
            ...
            supports_steering: true,
            steering_mode: SteeringMode::StepBoundary,
            reasoning_levels: comet_harness::hyper::catalog::REASONING_LEVELS.to_vec(),
        },
        Box::new(|| Ok(Arc::new(comet_harness::HyperHarness::new()) as Arc<dyn Harness>)),
    );
    registry
}
...
mod tests {
    ...
    fn default_registry_lists_mock_claude_and_codex_slots() {
        ...
        // Hyper resolves too.
        let hyper = registry.resolve(HarnessId::Hyper).unwrap();
        assert_eq!(hyper.id(), HarnessId::Hyper);
    }
    ...
    /// The Hyper lazy descriptor must match the resolved harness (same guard
    /// as the codex test above — picker rail must not flip after first use).
    #[test]
    fn hyper_lazy_descriptor_matches_resolved_harness() {
        let registry = default_registry();
        let before = registry
            .descriptors()
            .into_iter()
            .find(|d| d.id == HarnessId::Hyper)
            .unwrap();
        let resolved = registry.resolve(HarnessId::Hyper).unwrap();
        assert_eq!(before.name, resolved.display_name());
        assert_eq!(before.supports_steering, resolved.supports_steering());
        assert_eq!(before.steering_mode, resolved.steering_mode());
        assert_eq!(before.reasoning_levels, resolved.reasoning_levels());
    }
}
