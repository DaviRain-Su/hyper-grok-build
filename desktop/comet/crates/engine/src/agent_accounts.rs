...
pub struct AgentAccountsConfig {
    ...
    /// Hyper/grok home (`$GROK_HOME` or `~/.grok`) — holds `auth.json` (shared
    /// with the official `grok` CLI).
...
impl AgentAccountsConfig {
    ...
    /// Hyper/grok auth file: `$GROK_HOME/auth.json` (default `~/.grok/auth.json`).
    fn hyper_auth_file(&self) -> PathBuf {
...
enum LoginFlow {
    ...
    /// `hyper login` (or `grok login`) child — same spawn-CLI-and-poll model as
    /// Codex: an isolated `GROK_HOME` so the live `~/.grok` is never touched
    /// until the user switches; `poll_login` watches the isolated `auth.json`.
    Hyper {
        child: Arc<Mutex<Option<tokio::process::Child>>>,
        home: PathBuf,
        started_at: Instant,
...
impl AgentAccounts {
    ...
    pub async fn list(&self, force_usage: bool) -> Result<AgentAccountsSnapshot, EngineError> {
        ...
        for harness in [HarnessId::ClaudeCode, HarnessId::Codex, HarnessId::Hyper] {
    ...
    pub async fn activate(
        ...
        match harness {
            ...
            HarnessId::Hyper => self.activate_hyper(&slot)?,
            other => {
    ...
    async fn start_hyper_login(&self) -> Result<AgentLoginStart, EngineError> {
        ...
        let stale: Vec<String> = lock(&self.inner.flows)
            .iter()
            .filter(|(_, f)| matches!(f, LoginFlow::Hyper { .. }))
            .map(|(id, _)| id.clone())
            .collect();
        for id in stale {
            self.cancel_login(&id);
    ...
    pub async fn poll_login(&self, login_id: &str) -> Result<AgentLoginPoll, EngineError> {
        ...
        let (harness, home, exit, output) = match lock(&self.inner.flows).get(login_id) {
            ...
            Some(LoginFlow::Codex { home, exit, output, .. }) => {
                (HarnessId::Codex, home.clone(), exit.clone(), output.clone())
            }
            Some(LoginFlow::Hyper { home, exit, output, .. }) => {
                (HarnessId::Hyper, home.clone(), exit.clone(), output.clone())
            }
        };
        ...
        if let Some(code) = exited {
            ...
            let message = if code == Some(0) {
                format!("{:?} login finished without credentials.", harness)
            } else {
                lock(&output)
                    .trim()
                    .lines()
    ...
    pub fn cancel_login(&self, login_id: &str) {
        ...
        let (child, home) = match flow {
            Some(LoginFlow::Codex { child, home, .. })
            | Some(LoginFlow::Hyper { child, home, .. }) => (child, home),
            _ => return,
        };
