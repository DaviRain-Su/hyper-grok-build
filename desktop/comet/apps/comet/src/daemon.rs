//! `comet daemon …` — install/manage local `comet headless` as a background
//! service (systemd user unit / launchd). Cloud edge env vars are not used in
//! this local-link fork.
