...
```sh
curl -fsSL https://raw.githubusercontent.com/DaviRain-Su/hyper-grok-build/dev/install.sh | bash -s -- --version v0.2.117
```
...
1. Set the root [`VERSION`](VERSION) file to the **monorepo lockstep client
   ...
   `xai-grok-version`, currently `0.2.117`). CI compiles this into
...
| Asset | Example |
| ----- | ------- |
| macOS arm64 | `hyper-0.2.117-aarch64-apple-darwin.tar.gz` |
| macOS x86_64 | `hyper-0.2.117-x86_64-apple-darwin.tar.gz` |
| Linux x86_64 (glibc ≥2.17) | `hyper-0.2.117-x86_64-unknown-linux-gnu.tar.gz` |
| Linux arm64 (glibc ≥2.17) | `hyper-0.2.117-aarch64-unknown-linux-gnu.tar.gz` |
| Windows x86_64 | `hyper-0.2.117-x86_64-pc-windows-msvc.zip` |
| Checksums | `SHA256SUMS` |

The tag must match `VERSION` exactly (`v0.2.117` ↔ `0.2.117`) or the build fails.
