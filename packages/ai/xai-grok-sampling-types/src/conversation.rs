...
// engine in `packages/agent/xai-grok-compaction` operate over grok-build's
...
/// ([stream/messages.rs:340](packages/ai/xai-grok-sampler/src/stream/messages.rs))
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([packages/ai/xai-grok-sampler/src/stream/chat_completions.rs])
...
/// ([packages/ai/xai-grok-sampler/src/stream/messages.rs]) instead
...
mod tests {
    ...
    fn test_chat_completion_request_carries_reasoning_effort_top_level() {
        for (variant, expected) in [
            ...
            (crate::ReasoningEffort::Medium, "medium"),
            (crate::ReasoningEffort::High, "high"),
            // Chat Completions wire maps Xhigh → "max" (Kimi K3 / OpenAI alias).
