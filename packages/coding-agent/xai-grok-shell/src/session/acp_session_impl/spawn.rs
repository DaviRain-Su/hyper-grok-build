...
> {
    ...
    let chat_state_sampling_config = xai_grok_sampling_types::SamplingConfig {
        ...
        adapter_kind: sampling_config.adapter_kind,
        request_compat: sampling_config.request_compat.clone(),
        endpoint_path: sampling_config.endpoint_path.clone(),
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let spawn_snapshot = crate::session::SpawnSnapshot {
        applied_tool_overrides: session.effective_tool_overrides().await,
        scheduler_background_loops,
    };
