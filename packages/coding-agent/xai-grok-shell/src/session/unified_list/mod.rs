...
mod tests {
    ...
    /// `parse_list_req` is a no-op rewrite in this fork: the process chat
    /// mode is hard-off (`process_chat_mode_enabled()` always returns
    /// false), so client meta is passed through untouched in both states.
    #[test]
    #[serial_test::serial]
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        }
        {
            ...
            // Chat mode is hard-off: the env cannot enable the rewrite.
            assert!(!crate::agent::chat_modes::process_chat_mode_enabled());
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            ...
            assert_eq!(
                parsed.facet_filters.get(KIND_FACET_KEY),
                Some(&expected_build),
                "chat mode hard-off: client kind filter untouched"
            );
            assert_eq!(
                parsed.facet_filters.get("starred"),
                Some(&vec![serde_json::json!(true)]),
            ...
            assert_eq!(
                parsed.facet_filters.get(KIND_FACET_KEY),
                None,
                "absent client kind stays absent with chat mode off"
            );
            for (bad, expected_kind) in [
                ...
                assert_eq!(
                    ...
                    "no forced rewrite under chat-mode-off for: {bad}"
                );
            }
        }
    }
