...
Usage:
    ...
    python3 packages/coding-agent/xai-grok-shell/tests/memory_integration/run_tests.py

    # Run only fast (no-model) tests:
    python3 packages/coding-agent/xai-grok-shell/tests/memory_integration/run_tests.py --fast

    # Run a single test by name:
    python3 packages/coding-agent/xai-grok-shell/tests/memory_integration/run_tests.py test_fts_search_quality

    # Skip build (use pre-built binary):
    GROK_BINARY=/path/to/hyper python3 run_tests.py --fast
"""
...
def build_binary():
    """Build the hyper binary (xai-grok-pager-bin) with --release. Returns binary path."""
    repo = find_repo_root()
    if not repo:
        print(f"{R}Could not find repo root (no Cargo.toml + crates/ above cwd){N}")
        sys.exit(1)

    binary = os.path.join(repo, "target", "release", "hyper")
    ...
    print(f"{B}Building hyper (release)...{N}")
