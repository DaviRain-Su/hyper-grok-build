...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        //
        // Use real temp files instead of /dev/stdout + /dev/null: those Unix
        // device nodes do not exist on Windows (protoc then fails with
        // "/dev/stdout: No such file or directory"), and temp files work
        // everywhere.
        for proto in protos {
            let dep_file = tempfile::NamedTempFile::new()
                .context("create temp file for protoc --dependency_out")?;
            let desc_file = tempfile::NamedTempFile::new()
                .context("create temp file for protoc --descriptor_set_out")?;
            ...
            command
                .arg(format!(
                    "--dependency_out={}",
                    dep_path.to_str().context("dep_path not UTF-8")?
                ))
                .arg(format!(
                    "--descriptor_set_out={}",
                    desc_path.to_str().context("desc_path not UTF-8")?
                ));
            ...
            // continuation lines. Accept either the real desc path or the
            // historical `/dev/null:` prefix if a future caller reintroduces it.
            let mut lines = output.lines();
            let first_line = lines.next().context("protoc command output is empty")?;
            let desc_prefix = format!("{}:", desc_path.display());
            let rem = first_line
                .strip_prefix(&desc_prefix)
                .or_else(|| first_line.strip_prefix("/dev/null:"))
                // Windows protoc may normalize separators; try the path we passed
                // with forward slashes too.
                .or_else(|| {
                    let alt = format!("{}:", desc_path.to_string_lossy().replace('\\', "/"));
                    first_line.strip_prefix(&alt)
                })
                .with_context(|| {
                    format!(
                        "protoc dependency_out must start with `{desc_prefix}` \
                         (or /dev/null:): {output:?}"
                    )
                })?;
