...
///
/// Replaces the scrollback area when open.
/// Owns a `ListPaneState` for navigation, search, and visual-select.
/// Reads block content from the scrollback via `entry_id`.
pub struct BlockViewerPane {
    ...
    /// Text-level drag selection (character-precise, like the scrollback pane).
    /// Anchor and head are STABLE references to characters in the unified items list; they survive scrolling so copy includes off-screen content.
    /// Active between `Down(Left)` inside the content area and the matching `Up(Left)`.
    /// Stays visible after release until the next click.
...
///
/// References a position in the unified items list (preamble then body).
/// The position is the stable `item_idx` plus a display-column offset within that item's logical line text.
/// Stable across scroll: `item_idx` doesn't change when the viewport moves.
...
pub struct TextDrag {
    ...
    /// `true` while the mouse button is still down.
    /// Once released, the selection stays visible (for visual feedback) but isn't extended further.
    pub active: bool,
}
...
impl BlockViewerPane {
    ...
    pub fn tick(&mut self, entry: &ScrollbackEntry) -> bool {
        ...
        if self.was_running && !entry.is_running {
            self.was_running = false;
            self.list_state.disable_follow_permanently();
            // Select the last item by stable_id
    ...
    pub fn handle_key(&mut self, key: &KeyEvent) -> bool {
        ...
        // Pass the same unified vec the layout was prepared against so selection / scroll math stays in bounds
        // Any consumed key (navigation, search, copy, etc.) also clears a sticky mouse text selection so the highlight doesn't linger across actions
    ...
    ///
    /// Call after `handle_key` or `handle_mouse`.
    /// The caller is responsible for clipboard operations and toast display.
    /// Returns `None` if no copy pending.
    ...
    ///
    /// Drag-to-highlight: a left-button drag inside the content area paints a character-precise selection (same model as the scrollback pane).
    /// Anchor and head are STABLE references (item index plus char offset in the item's logical line), so the selection survives scrolling.
    /// The auto-copy on release therefore includes any content the drag covered even if it's now off-screen.
    /// The selection stays visible after release until the next click.
    pub fn handle_mouse(&mut self, kind: MouseEventKind, col: u16, row: u16) -> bool {
        ...
        match kind {
            MEK::Down(MouseButton::Left) if in_content(col, row) => {
                // Start a fresh text selection anchored at the clicked character
                // Also clear any keyboard visual-mode highlight so the user only sees the new mouse selection
                self.list_state.exit_visual_mode();
            ...
            MEK::Drag(MouseButton::Left) => {
                if let Some(drag) = self.text_drag
                    && drag.active
                {
                    // Auto-scroll when the cursor drags above or below the content area (same idea as scrollback drag_autoscroll)
                    ...
                    // Clamp the cursor to the content area then map to an item endpoint so the selection extends as we scroll
            ...
            MEK::Up(MouseButton::Left) => {
                if let Some(drag) = self.text_drag
                    && drag.active
                {
                    // Update head to the release position
                    // The last Drag(Left) may trail the actual release by a few characters, causing 1-2 missing words at the end
            ...
            // Any other button-down (including a left-click outside the content area) clears a sticky selection
            // The highlight then doesn't linger after the user moves on
            MEK::Down(_) => {
                self.text_drag = None;
                self.list_state
    ...
    /// Translate a screen position inside the content area into a stable `TextEndpoint`.
    /// The endpoint is an item index plus a display-column offset within the item's logical line text.
    ///
    /// Returns `None` if the position doesn't map to any item (e.g. blank space below the last visual row of the content).
    /// The result is clamped: clicks beyond the end of an item's last sub-row map to `col = end_of_logical_line`.
    /// Clicks past the rightmost char on a sub-row map to that sub-row's end column.
    fn screen_to_endpoint(
        ...
        // Use joiners to account for spaces dropped at each break point; without them, absolute_col drifts by one or more chars per wrap break
        ...
        for (i, line) in wrapped.iter().enumerate() {
            ...
            if i == sub_row as usize {
                if col_in_sub >= line_w && i + 1 < wrapped.len() {
                    // Cursor at/past the right edge of a non-last sub-row: snap to end of logical line
        ...
        // (This also handles items shorter than the layout item_height; shouldn't happen, but keeps the click meaningful.)
        Some(TextEndpoint {
            item_idx,
            col: absolute_col,
        })
    ...
    /// Effective wrap width used by the ListPane for this frame: pane width minus the scrollbar gutter when a scrollbar is shown.
    ...
    /// Wrap one item's content, returning wrapped lines and the per-break joiners (spaces dropped at each break point).
    /// The joiner widths are needed to correctly map screen positions to display-column offsets in the original unwrapped text.
    ...
    /// Column-precise: slices partial lines at the start/end of the selection, full lines in the middle.
    ...
    ///
    /// Called by the modal renderer AFTER `render_content` has drawn the items.
    /// Walks the visible portion of the selection (off-screen portions are hidden, but they're still in the copy text).
    /// Inverts fg/bg on every cell that falls inside the per-sub-row column range.
    pub fn render_text_drag_overlay(&self, buf: &mut ratatui::buffer::Buffer) {
        ...
        for idx in start.item_idx..=end.item_idx {
            ...
            let (wrapped, joiners) = self.wrap_item_with_joiners(item, wrap_w);
            // Selection covers chars [item_lo .. item_hi) of this item's logical line.
            ...
            // Walk this item's sub-rows
            // acc_col tracks the absolute column in the original text, including joiner widths
...
/// Painted width of a `Line` in terminal cells, clamped to `u16::MAX`.
/// Counts per-grapheme cells (like `types::str_display_cells`) so it agrees with `visual_col_to_logical_col` on ligature rows (Arabic lam-alef).
...
impl BlockViewerPane {
    ...
    /// `prepend_lines` are pre-wrapped header lines to render at the top of the scrollable content (preamble and a blank separator).
    /// They get their own stable IDs in the high half of the u64 space so they don't conflict with content item IDs.
