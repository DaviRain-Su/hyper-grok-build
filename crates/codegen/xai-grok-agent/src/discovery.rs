...
/// The full list is defined by [`BuiltinAgentName::subagent_variants()`].
...
mod tests {
    ...
    #[test]
    fn test_merge_returns_5_builtins_when_no_user_agents() {
    ...
    fn test_merge_all_toggled_off_returns_empty() {
        let toggle = HashMap::from([
            ...
            ("oracle".to_string(), false),
            ("xdotcom".to_string(), false),
        ]);
        let entries = merge_subagents(vec![], &toggle);
