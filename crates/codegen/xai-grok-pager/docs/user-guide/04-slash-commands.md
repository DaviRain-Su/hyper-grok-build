...
```text
/login                 # xAI Grok / configured enterprise OIDC (always the default)
...
Log out and return to the login screen. Use flags / subcommands for third-party scopes (`--kimi`, `--openai`, `--claude`, `--all`, or `/logout provider <platform>`).
