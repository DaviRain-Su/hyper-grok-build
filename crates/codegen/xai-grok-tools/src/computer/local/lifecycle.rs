...
mod tests {
    ...
    async fn a_collection_claim_requires_the_child_to_have_been_waited_on() {
        ...
        let (mut child, group) = scope.spawn(command).expect("spawn enrolled `true`");
        let mut lifecycle = exiting();
        ...
        // Release the group owner after reap so scope teardown cannot target a
        // recycled pid/process-group id.
        drop(group);
        lifecycle.finish_output(Collection::of(&child));
