...
- **13 preset themes** — A curated collection layered on top of the original five, distinguished primarily by background color: ten dark (`everforest`, `nord`, `dracula`, `gruvbox`, `catppuccin-mocha`, `solarized-dark`, `deep-ocean`, `ember`, `midnight-oled`, `base16-default-dark`) and three light (`solarized-light`, `catppuccin-latte`, `paper`). Pick via `/theme <name>`, Settings → Appearance → Theme, or the `auto` dark/light pairings. All are truecolor (RGB) and fall back to Grok Night on 256/16-color terminals, including persisted choices loaded after moving to a lower-color terminal. Each preset is defined from a compact palette expanded through a shared builder so semantic roles (error=red, success=green, sunken code blocks, scrollbar contrast) stay consistent across the set. Base16 Default Dark uses stable theme ID 18; minimal mode remains terminal-native and therefore follows the terminal's own Base16 palette.
- **Translations for the preset themes** — `settings.{theme,auto_dark_theme,auto_light_theme}.choice.*.description` for the original nine dark + three light presets across all nine non-English locales (de, es, fr, ja, ko, pt-BR, ru, zh-CN, zh-TW); previously they fell back to English.
